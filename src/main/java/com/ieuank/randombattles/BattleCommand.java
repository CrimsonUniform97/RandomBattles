	    ...
	    if(astring.length == 0)
	    {
	    	...
	    	if(BattleRegistry.getBattle(p) == null) {
		    	...
		    	if(this.mod.playerQueue.size() > 1) {
		    		...
		    		RandomBattles.logger.log(Level.INFO, p.getDisplayName() + " joined the battle queue (" + this.mod.playerQueue.size() + ")");
	    		...
	    		RandomBattles.logger.log(Level.INFO, p.getDisplayName() + " tried joining battle queue while in a battle.");
	    	}
	    } else {
